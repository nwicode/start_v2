...
//import { LayoutService } from '../../../framework/core';
...
export class SubheaderService implements OnDestroy {
  ...
  constructor(private layout: LayoutService, private title: Title, private meta: Meta, private translate: TranslationService, private router: Router, private applicationService: ApplicationService) {
  ...
  setBreadcrumbs(breadcrumbs: BreadcrumbItemModel[] = []) {
    ...
    //if (breadcrumbs[0]) this.title.setTitle(this.translate.translatePhrase(breadcrumbs[0].title)+" - " + authSettions.domain);

    let routeMatch = this.router.url.match(/constructor\/(\d+)/);
    if (routeMatch && routeMatch.length > 1) {
      let currentAppId = Number(routeMatch[1]);

      if (this.applicationName && this.applicationId == currentAppId) {
        if (breadcrumbs[0]) this.title.setTitle(this.translate.translatePhrase(breadcrumbs[0].title) + " - " + this.applicationName + " - " + window.location.hostname);
      } else {
        this.applicationService.getApplicationById(currentAppId).then(result => {
          this.applicationName = result.name;
  ...
  setTitle(title: string = '') {
    ...
    //this.title.setTitle(this.translate.translatePhrase(title)+" - " + authSettions.domain);
  }
