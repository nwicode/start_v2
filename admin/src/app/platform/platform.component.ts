...
export class PlatformComponent implements OnInit, AfterViewInit {
  ...
  public initAside: boolean = true;

  constructor(private initService: LayoutInitService, private layout: LayoutService, private user: UserService, private notificationService: NotificationsService,  private route: ActivatedRoute,private router: Router,) {
    
    if (this.router.url=="/subscriptions") this.initAside = false;
    
    this.initService.init(this.initAside);
